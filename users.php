...
<body>
	...
	<div class="mx-auto col-sm-12 main-section" id="myTab" role="tablist">
		<ul class="nav nav-tabs justify-content-end ">
			...
			<li class="nav-item">
			<a class="nav-link active" id="list-tab" data-toggle="tab" href="#list" role="tab" aria-controls="list" aria-selected="false">Lista de equipos</a>
			</li>
		</ul>
		<div class="tab-content" id="myTabContent">
					...
					<div class="card-body">
							<div class="table-responsive">
								<table id="userList" class="table table-bordered table-hover table-striped">
                                    ...
                                    $query = $conn->query("SELECT * FROM inventario_laboratorio");
			...
			<div class="tab-pane fade" id="form" role="tabpanel" aria-labelledby="form-tab">
				...
				<div class="tab-pane fade" id="form1" role="tabpanel" aria-labelledby="form-tab">
					<div class="card">
							...
							<div class="card-body">
								...
								<?php
                                require 'Conexion_BD.php';
                                $query = $conn->query("SELECT * FROM usuarios WHERE username = '$identificar'");
										...
										<div class="modal-content">
											...
											<div class="modal-body">
												<form id="formularios" action ="reserva.php" method = "POST" autocomplete="off">
													...
													<div class="col-lg-9">
														<input class="form-control" type="number" name="Nuevacantidad" required min="0" pattern="\d+"  max="<?php echo $cantidad ?>">
													</div>
													<label for="fecha">Fecha Inicio:</label>
                                                    <div class="col-lg-9">
                                                    <input class="form-control" type="date" id="DateStartInput" name="fecha1" required min="<?php echo date('Y-m-d'); ?>" max="2023-12-31"><br>
                                                    </div>

                                                    <label for="fecha2">Fecha de Entrega:</label>
                                                    <div class="col-lg-9">
                                                    <input class="form-control" type="date" id="DateEntreInput" name="fecha2" required min="<?php echo date('Y-m-d'); ?>" max="2023-12-31" disabled><br>
                                                    </div>

                                                    <script>
                                                    // Obtener elementos del DOM
                                                    var startDateInput = document.getElementById('DateStartInput');
                                                    var endDateInput = document.getElementById('DateEntreInput');

                                                    // Agregar evento de cambio a la fecha de inicio
                                                    startDateInput.addEventListener('change', function() {
                                                        ...
                                                        // Formatear la fecha mínima para el campo de fecha de entrega
                                                        var minEndDate = startDate.toISOString().split('T')[0];
                                                        
                                                        // Habilitar la fecha de entrega y establecer la fecha mínima
				...
				<table id="reservList1" class="table table-bordered table-hover table-striped">
					...
					<?php 
                    require 'Conexion_BD.php';
                    $query = $conn->query("SELECT * FROM reservaciones WHERE user_id = '$identificar'");
                    $result = $query->rowCount();
                    if ($result > 0) {
                        while ($data = $query->fetch(PDO::FETCH_ASSOC)) {
                            ?>
				...
				<table id="reservList2" class="table table-bordered table-hover table-striped">
					...
					<?php 
                    require 'Conexion_BD.php';
                    $query = $conn->query("SELECT * FROM reservaciones");
                    $result = $query->rowCount();
                    if ($result > 0) {
                        while ($data = $query->fetch(PDO::FETCH_ASSOC)) {
                            ?>
